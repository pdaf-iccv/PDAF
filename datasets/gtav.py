...
class GTAV(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_path = mask_path.replace(".png", "_trainId.png")
        img, mask = Image.open(img_path).convert('RGB'), m.imread(mask_path)
        img_name = os.path.splitext(os.path.basename(img_path))[0]

        # while (img.size[1], img.size[0]) != mask.shape[:2]:
        #     os.remove(img_path)
        #     os.remove(mask_path)
        #     print(f"Error!!, {img.size}, {mask.shape}, {img_name}")
        #     print(f"Dropping , {str(index)}")
        #     # index = index + 1
